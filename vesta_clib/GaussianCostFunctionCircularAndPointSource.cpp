...
GaussianCostFunctionCircularAndPointSource
::GaussianCostFunctionCircularAndPointSource(float* u, float* v,
...
bool GaussianCostFunctionCircularAndPointSource
           ...
           double** jacobians) const
{
	...
	double point_source_flux = parameters[4][0];
	...
	for(int uvrow = 0; uvrow < _nrow; uvrow++)
	{
		for(int chan = 0; chan < _nchan; chan++)
		{
			for(int pol = 0; pol < _nstokes; pol++)
			{
				...
				{
					residuals[2*index+0] = sqrt_weight*(_V_real[index] - V_mod_real - point_source_flux*pos_real);
					residuals[2*index+1] = sqrt_weight*(_V_imag[index] - V_mod_imag - point_source_flux*pos_imag);
